...
def show_animation(nb_frames, min_level, max_level, order, directory):
    ...
    for i in range(nb_frames):
        ...
        marker_level = dict(symbol='line-ns', line_color='rgb(85,168,104)', line_width=1, size=4) 
    
        frames.append(go.Frame(data=[go.Scatter(x=xqexa, y=uqexa, name="quasi exa. sol."), 
                                     go.Scatter(x=xqexa, y=u_recons, name="sol."),
                                     go.Scatter(x=myxmr, y=mylevelmr, mode="lines+markers", marker=marker_level, name="level")], name=name))
    ...
    fig.add_trace(go.Scatter(x=frames[7].data[2].x, y=frames[7].data[2].y, mode='lines+markers', marker=marker_level, name="level "), row=2, col=1)
 
    sliders = [dict(currentvalue={'prefix': 't = '}, active=7, visible=True, steps=steps)]

    title=f'Burgers 1D, eps = {1e-3:.0e},  min level = {min_level}, max level = {max_level}'
    legend = dict(x=0.80, y=0.99, bgcolor='rgba(0,0,0,0)', font=dict(size=14))
    fig.update_layout(legend=legend, height=600, width=1000, sliders=sliders, title=title, 
                      margin=dict(l=20, r=20, t=100, b=20))
    fig.update(frames=frames)
    fig.update_xaxes(range = [xmr[0], xmr[-1]], row=2)
    fig.update_yaxes(range = [min_lev-1,max_lev+1], row=2)
    fig.show()

show_animation(8, 3, 12, 1, 'burgers_mr_mlf_eps_1e-3')
```