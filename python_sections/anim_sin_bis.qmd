...
def show_animation(nb_frames):
    ...
    for i in range(0, nb_frames):
        ...
        frames.append(go.Frame(data=[go.Scatter(x=x, y=u, name=f"exa. sol. (t = {t[i]:.3f})"), 
    ...
    fig.update_layout(updatemenus=[dict(type="buttons", buttons=buttons, direction="left", x=0.5, y=-0.1)], 
                      title=f'Burgers, eps = {1e-3:1e},  min level = {min_level}, max level = {max_level}', height=550, legend=dict( font=dict(size=16), x=0.01, y=0.97, bgcolor='rgba(0,0,0,0)', tracegroupgap=250, groupclick="toggleitem"), width=1000)
