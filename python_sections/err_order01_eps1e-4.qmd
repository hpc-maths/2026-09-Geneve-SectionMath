...
def compute_err(eps):
    ...
    filename = './data/order01/burgers_uniform/burgers_level_'+str(ilevel)+'_'+str(ilevel)+'_ite_7.h5'
    ...
    for ilevel in [12, 11, 10, 9, 8, 7]:
        ...
        filename = './data/order01/burgers_mr_eps_'+eps+'/burgers_level_'+str(min_level)+'_'+str(ilevel)+'_ite_7.h5'
        xmr, umr, levelmr = read_frame(filename, 'u')
        umr_recons = recons(xmin, xmax, min_level, ilevel, xmr, umr, levelmr)

        filename = './data/order01/burgers_mr_mlf_eps_'+eps+'/burgers_level_'+str(min_level)+'_'+str(ilevel)+'_ite_7.h5'
    ...
    fig.add_trace(go.Scatter(x=ldx, y=2e-1*np.array(ldx)**order, mode="lines", line_color='black', name=f"slope {order}"))
