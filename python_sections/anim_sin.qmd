...
def show_animation(nb_frames):
    ...
    fig.add_trace(go.Scatter(x=frames[0].data[0].x, y=frames[0].data[0].y, name=f"exa. sol. (t = {t[0]:.3f})"))
    ...
    fig.update_layout(updatemenus=[dict(type="buttons", buttons=buttons, direction="left", x=0.5, y=-0.1)], 
                      title=f'Burgers, eps = {1e-3:.2e},  min level = {min_level}, max level = {max_level}', height=650, legend=dict( font=dict(size=16), x=0.01, y=0.97, bgcolor='rgba(0,0,0,0)', tracegroupgap=250, groupclick="toggleitem"), width=1000)
